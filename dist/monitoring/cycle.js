...
const kener_client_1 = require("./kener-client");
    ...
    const snapshots = await Promise.all(exports.MONITORED_SERVICES.map(async (svc) => {
        ...
        try {
            ...
            if (prev && prev !== status) {
                ...
                (0, kener_client_1.handleStatusTransition)(svc.name, prev, status, `HTTP ${resp.status} en ${ms}ms`);
            }
            ...
            if (prev !== 'down') {
                ...
                (0, kener_client_1.handleStatusTransition)(svc.name, prev, 'down', reason);
            }
