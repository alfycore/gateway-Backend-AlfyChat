...
exports.registerServersHandlers = registerServersHandlers;
const emit_1 = require("../utils/emit");
function registerServersHandlers(socket, io, serviceProxy) {
    ...
    socket.on('SERVER_JOIN', async (data) => {
        try {
            ...
            (0, emit_1.emitError)(socket, 'SERVER_ERROR', error);
        }
    });
    // Quitter un serveur
    ...
    socket.on('SERVER_CREATE', async (data) => {
        try {
            ...
            (0, emit_1.emitToSocket)(socket, 'SERVER_CREATE_SUCCESS', server);
        }
        catch (error) {
            (0, emit_1.emitError)(socket, 'SERVER_ERROR', error);
        }
    ...
    socket.on('CHANNEL_CREATE', async (data) => {
        try {
            ...
            (0, emit_1.emitError)(socket, 'CHANNEL_ERROR', error);
        }
    });
    ...
    socket.on('MEMBER_KICK', async (data) => {
        try {
            ...
            (0, emit_1.emitError)(socket, 'MEMBER_ERROR', error);
        }
    });
...
//# sourceMappingURL=servers.handler.js.map