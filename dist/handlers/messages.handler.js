...
function registerMessageHandlers(socket, io, serviceProxy) {
    ...
    socket.on('MESSAGE_CREATE', async (data) => {
        try {
            ...
            (0, emit_1.emitError)(socket, 'MESSAGE_CREATE_ERROR', error);
        }
    });
    ...
    socket.on('MESSAGE_UPDATE', async (data) => {
        try {
            ...
            (0, emit_1.emitError)(socket, 'MESSAGE_UPDATE_ERROR', error);
        }
    });
    ...
    socket.on('MESSAGE_DELETE', async (data) => {
        try {
            ...
            (0, emit_1.emitError)(socket, 'MESSAGE_DELETE_ERROR', error);
        }
    });
    ...
    socket.on('REACTION_ADD', async (data) => {
        try {
            ...
            (0, emit_1.emitError)(socket, 'REACTION_ERROR', error);
        }
    });
