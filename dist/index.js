...
const api_version_1 = require("./http/api-version");
const service_keys_1 = require("./state/service-keys");
const cycle_1 = require("./monitoring/cycle");
const anomaly_1 = require("./monitoring/anomaly");
...
const lb_routes_1 = require("./http/lb.routes");
const admin_routes_1 = require("./http/admin.routes");
const sfu_handler_1 = require("./handlers/sfu.handler");
...
const stats_routes_1 = require("./http/stats.routes");
...
app.use((req, res, next) => {
    ...
    if (origin &&
        env_1.IS_PRODUCTION &&
        !env_1.allowedOrigins.includes(origin) &&
        !/^http:\/\/localhost(:\d+)?$/.test(origin)) {
        logger_1.logger.warn(`CORS: origine non autorisée — ${origin}`);
...
app.use(api_version_1.apiVersionMiddleware);
...
app.use(anomaly_1.anomalyMiddleware);
...
app.use(async (req, res, next) => {
    ...
    const hit = env_1.AUTH_BRUTEFORCE_PATHS.some((p) => req.path === p || req.path.startsWith(p + '/'))
        || env_1.AUTH_BRUTEFORCE_REGEX.some((re) => re.test(req.path));
    if (!hit)
        return next();
    const ip = (0, helpers_1.getClientIP)(req);
    try {
...
(0, lb_routes_1.registerLBRoutes)(app);
(0, stats_routes_1.registerStatsRoutes)(app);
...
app.post('/api/servers/:serverId/claim-admin', async (req, res) => {
    const { serverId } = req.params;
    const userId = (0, helpers_1.extractUserIdFromJWT)(req.headers.authorization);
    try {
        const serversBase = (0, helpers_1.getServiceUrl)('servers', env_1.SERVERS_URL);
        ...
        const data = await (0, helpers_1.safeJson)(response) ?? { error: 'Réponse vide' };
        if (!response.ok)
            return res.status(response.status).json(data);
        // Sync le server-node sans bloquer la réponse
        if (userId && connections_1.connectedNodes.has(serverId)) {
            (async () => {
                try {
                    // 1. Mettre à jour ownerId dans serverSelf du node
                    await (0, forward_1.forwardToNode)(serverId, 'SERVER_UPDATE', { ownerId: userId }, 5_000);
                    // 2. Broadcaster SERVER_UPDATE à tous les clients du serveur
                    runtime_1.runtime.io.to(`server:${serverId}`).emit('SERVER_UPDATE', {
                        type: 'SERVER_UPDATE',
        ...
        logger_1.logger.error({ err }, 'Erreur claim-admin gateway hook:');
...
app.all('/api/oauth2/*', (req, res) => (0, proxy_2.proxyRequest)((0, helpers_1.getServiceUrl)('bots', env_1.BOTS_URL), req, res, env_1.BOTS_URL, 'bots'));
app.all('/api/oauth2', (req, res) => (0, proxy_2.proxyRequest)((0, helpers_1.getServiceUrl)('bots', env_1.BOTS_URL), req, res, env_1.BOTS_URL, 'bots'));
// Routes Bots
...
app.post('/internal/call-quality', express_1.default.json(), (req, res) => {
    const secret = req.headers['x-internal-secret'];
    if (!secret || secret !== env_1.INTERNAL_SECRET) {
    ...
    (0, sfu_handler_1.broadcastQualityUpdate)(io, callId, tier, participantCount, tierParams);
...
io.use(async (socket, next) => {
    try {
        ...
        (0, anomaly_1.attachAnomalyWsHooks)(socket, decoded.userId);
        next();
    }
    catch (error) {
...
async function handleServerMentions(opts) {
    ...
    const ids = Array.isArray(mentionedUserIds)
        ? mentionedUserIds.filter((id) => id && id !== senderId)
        : [];
    if (ids.length === 0)
        return;
 ...
 */
async function getUserPermBits(userId, serverId) {
    try {
        const server = await serviceProxy.servers.getServer(serverId);
        const ownerId = server?.ownerId || server?.owner_id;
        if (ownerId === userId)
            return { isOwner: true, perms: 0xFFF };
        const members = await serviceProxy.servers.getMembers(serverId);
        const member = (members || []).find((m) => (m.userId || m.user_id || m.id) === userId);
        if (!member)
        ...
        logger_1.logger.warn({ err }, 'getUserPermBits error');
        return { isOwner: false, perms: 0 };
    }
}
...
io.on('connection', async (socket) => {
    ...
    try {
        ...
        try {
            ...
            if (cached) {
                for (const sid of cached.serverIds)
                    socket.join(`server:${sid}`);
                for (const cid of cached.channelIds)
                    socket.join(`channel:${cid}`);
                logger_1.logger.info(`${user.username} restored ${cached.serverIds.length} servers + ${cached.channelIds.length} channels from Redis cache`);
            }
        }
        catch { /* non bloquant */ }
        ...
        if (userServers && Array.isArray(userServers)) {
            const allServerIds = [];
            const allChannelIds = [];
            await Promise.all(userServers.map(async (srv) => {
                ...
                try {
                    let channels = [];
                    try {
                        const chResult = await (0, forward_1.forwardToNode)(sid, 'CHANNEL_LIST', {});
                        channels = chResult?.channels || [];
                    }
                    catch {
                        const server = await serviceProxy.servers.getServer(sid);
            ...
            logger_1.logger.info(`${user.username} auto-joined ${allServerIds.length} server rooms, ${allChannelIds.length} channel rooms`);
        }
    }
    catch (error) {
    ...
    const restoredText = restoredPresence?.text ?? user.customStatus ?? null;
    ...
    socket.on('PREFERENCES_UPDATE', (data) => {
        ...
        try {
            ...
            if (data.recipientId) {
                ...
                if (senderUserRoom && senderUserRoom.size > 1) {
                    serviceProxy.users.getUser(data.recipientId).then((recipientUser) => {
            ...
            if (!data.recipientId && !conversationId.startsWith('dm_')) {
                ...
                const preview = data.content && !data.content.startsWith('ecdh:')
                    ? (data.content.length > 80 ? data.content.slice(0, 80) + '…' : data.content)
                    : '📎 Nouveau message';
                serviceProxy.messages.getConversationParticipants(conversationId).then((result) => {
            ...
            serviceProxy.messages.createMessage({
                ...
                const mentionedUserIds = Array.isArray(data.mentionedUserIds)
                    ? data.mentionedUserIds.filter((id) => id && id !== userId)
                    : [];
                if (mentionedUserIds.length > 0) {
                    ...
                    for (const mentionedId of mentionedUserIds) {
                        ...
                        redis.isUserOnline(mentionedId).then((isOnline) => {
                            if (isOnline) {
                                io.to(`user:${mentionedId}`).emit('MENTION_NOTIFY', mentionPayload);
                            }
                            else {
    ...
    socket.on('message:edit', async (data) => {
        try {
            ...
            rate_limit_1.messageRateLimit.set(userId, recent);
            const v = (0, validation_1.validateMessageContent)(data?.content);
            const updated = await serviceProxy.messages.updateMessage(data.messageId, v.content, userId, data.senderContent, data.e2eeType);
            if (!updated) {
        ...
        try {
            ...
            if (conversationId && conversationId.startsWith('dm_')) {
                ...
                if (!parts.includes(userId)) {
                    logger_1.logger.warn(`DM_ARCHIVE_PEER_RESPONSE rejeté: ${userId} n'est pas participant de ${conversationId}`);
                    return;
                }
            }
            ...
            logger_1.logger.info(`📨 Peer ${userId} a fourni ${sanitizedMessages.length} msg archivés pour ${requesterId}`);
        }
        catch (error) {
            emitError(socket, 'DM_ARCHIVE_ERROR', error);
        }
    ...
    socket.on('CALL_INITIATE', async (data, callback) => {
        try {
            logger_1.logger.info(`CALL_INITIATE DM de ${userId} (${user?.username}) vers ${data.recipientId || '?'} type=${data.type}`);
            ...
            if (!callId)
                throw new Error('Calls service returned no call ID');
            ...
            logger_1.logger.info(`Appel DM créé: ${callId}`);
        }
        catch (error) {
    ...
    socket.on('CALL_INITIATE_GROUP', async (data, callback) => {
        try {
            logger_1.logger.info(`CALL_INITIATE_GROUP de ${userId} (${user?.username}) channel=${data.channelId} type=${data.type}`);
            if (!data.channelId) {
                if (typeof callback === 'function')
                    callback({ error: 'channelId requis' });
                return;
            }
            ...
            logger_1.logger.info(`Appel groupe créé: ${callId} channel=${data.channelId}`);
        }
        catch (error) {
            emitError(socket, 'CALL_ERROR', error);
            if (typeof callback === 'function')
                callback({ error: 'Failed to initiate group call' });
        }
    });
    ...
    socket.on('CALL_JOIN', async (data, callback) => {
        try {
            ...
            if (!call || call.status === 'ended') {
                if (typeof callback === 'function')
                    callback({ error: 'Appel introuvable ou terminé' });
                return;
            }
            ...
            const existingParticipants = socketsInRoom
                ...
                .map((s) => ({
                userId: s.userId,
                userName: s.user?.displayName || s.user?.username || 'Utilisateur',
            ...
            const callCat = call.callCategory || 'group';
            let callMode = 'p2p';
            if (callCat === 'server') {
                callMode = 'sfu';
            }
            else if (callCat === 'group' && newCount > sfu_handler_1.GROUP_SFU_THRESHOLD) {
                callMode = 'sfu';
                ...
                if (currentMode !== 'sfu') {
                    ...
                    try {
                        await serviceProxy.media.createRoom(callId, 'group');
                    }
                    catch { /* ok si existe déjà */ }
                    // Notifier tous les participants du switch P2P→SFU
                    Promise.resolve().then(() => __importStar(require('./handlers/sfu.handler'))).then(({ broadcastModeSwitch }) => {
            ...
            logger_1.logger.info(`${userId} a rejoint l'appel ${callCat} ${callId} (${newCount} participants, mode=${callMode})`);
        }
        catch (error) {
            if (error?.statusCode === 404) {
                socket.emit('CALL_END', { type: 'CALL_END', payload: { callId: data.callId }, timestamp: new Date() });
                if (typeof callback === 'function')
                    callback({ error: 'Appel déjà terminé' });
            }
            else {
                emitError(socket, 'CALL_ERROR', error);
                if (typeof callback === 'function')
    ...
    socket.on('CALL_LEAVE', async (data) => {
        try {
            ...
            logger_1.logger.info(`${userId} a quitté l'appel ${data.callId}`);
        }
        catch (error) {
            emitError(socket, 'CALL_ERROR', error);
        }
    ...
    socket.on('CALL_INITIATE_SERVER', async (data, callback) => {
        try {
            logger_1.logger.info(`CALL_INITIATE_SERVER de ${userId} channel=${data.channelId} serveur=${data.serverId}`);
            if (!data.channelId || !data.serverId) {
                if (typeof callback === 'function')
                    callback({ error: 'channelId et serverId requis' });
                return;
            }
            ...
            if (currentCallSockets.length >= limit) {
                if (typeof callback === 'function')
                    callback({ error: 'CALL_FULL', maxParticipants: limit });
                return;
            }
            ...
            try {
                ...
                logger_1.logger.error({ err: sfuErr }, 'Impossible de créer la salle SFU pour appel serveur:');
            }
            ...
            io.to(`channel:${data.channelId}`).emit('CALL_INCOMING', {
                ...
                payload: {
                    ...
                    isGroup: false,
                    initiatorId: userId,
                    callerName: socket.user?.displayName || socket.user?.username || userId,
                    callType: data.type || 'voice',
                },
                timestamp: new Date(),
            });
            ...
            emitError(socket, 'CALL_ERROR', error);
            if (typeof callback === 'function')
                callback({ error: 'Impossible de créer l\'appel serveur' });
        }
    });
    // ── SFU Signaling (relay vers media-server) ────────────────────────────────
    (0, sfu_handler_1.registerSfuHandlers)(socket, io, serviceProxy);
    // ============ SERVEURS P2P ============
            ...
            try {
                ...
                if (!alreadyMember) {
                    ...
                    if (!isPublic) {
                        ...
                        let inviteOk = false;
                        try {
                            const r = await (0, forward_1.forwardToNode)(data.serverId, 'INVITE_VERIFY', { code });
                            inviteOk = !!(r && r.valid !== false && !r.error);
                        }
                        catch {
                            try {
                                const invite = await serviceProxy.servers.resolveInvite(code);
    ...
    socket.on('SERVER_SELF_JOIN', async (data) => {
        try {
            ...
            try {
                // Charger les channels depuis le node
                const chResult = await (0, forward_1.forwardToNode)(serverId, 'CHANNEL_LIST', {});
                if (chResult?.channels) {
                    for (const ch of chResult.channels) {
                        socket.join(`channel:${ch.id}`);
                    }
            ...
            logger_1.logger.info(`${user.username} self-joined server ${serverId} via HTTP → socket rooms updated`);
        }
        catch (error) {
            emitError(socket, 'SERVER_ERROR', error);
        }
    ...
    socket.on('ROLE_UPDATE', async (data) => {
        try {
            ...
            const actor = await getUserPermBits(userId, data.serverId);
            let safePerms = clean.permissions;
                ...
                if (!actor.isOwner) {
                    try {
                        ...
                        if (safeRoleIds.length !== roleIds.length) {
                            logger_1.logger.warn(`MEMBER_UPDATE escalation blocked: ${userId} tried to assign superior roles on server ${serverId}`);
                        }
                    }
                    catch (e) {
                        if (typeof callback === 'function')
                            callback({ error: 'ROLE_CHECK_FAILED' });
                        return;
                    }
                }
            }
    ...
    socket.on('SERVER_UPDATE_NODE', async (data) => {
        try {
            ...
            const updated = await serviceProxy.servers.updateServer(data.serverId, updates, userId, token);
    ...
    socket.on('INVITE_VERIFY', async (data, callback) => {
        try {
            if (!(0, rate_limit_1.checkInviteVerifyRate)(userId)) {
                if (typeof callback === 'function')
    ...
    socket.on('CHANNEL_JOIN', async (data) => {
        try {
            ...
            let isMember = false;
            const nodeSocket = (0, forward_1.getNodeSocket)(serverId);
            if (nodeSocket) {
                const result = await (0, forward_1.forwardToNode)(serverId, 'MEMBER_CHECK', { userId });
            ...
            socket.join(`channel:${channelId}`);
            logger_1.logger.info(`${userId} rejoint channel:${channelId}`);
        }
        catch (err) {
            logger_1.logger.warn({ err }, 'CHANNEL_JOIN permission check error');
        }
    });
    ...
    socket.on('PRESENCE_UPDATE', async (data) => {
        try {
            ...
            if (!data?.status || !VALID_STATUSES.includes(data.status))
                return;
            const emoji = data.emoji ? String(data.emoji).trim().slice(0, 2) || null : null;
            ...
            serviceProxy.users.updateStatus(userId, data.status, text, emoji)
                .catch((err) => logger_1.logger.warn({ err }, `updateStatus DB failed for ${userId}`));
            const friends = await serviceProxy.friends.getFriends(userId);
    ...
    socket.on('PROFILE_UPDATE', async (data) => {
        try {
            ...
            try {
                ...
                logger_1.logger.warn({ err: e }, 'Erreur notification profil serveurs:');
            }
        }
        catch (error) {
    ...
    socket.on('CHANGE_USERNAME', async (data) => {
        try {
            ...
            const token = socket.handshake.auth.token || socket.handshake.headers.authorization?.replace('Bearer ', '');
            const result = await serviceProxy.users.changeUsername(userId, newUsername, password, token);
            if (!result?.success) {
                socket.emit('CHANGE_USERNAME_ERROR', { error: result?.error || 'Erreur lors du changement' });
                return;
            }
            // Mettre à jour le user local du socket
            Object.assign(socket, { user: { ...user, username: newUsername } });
            ...
            try {
                ...
                logger_1.logger.warn({ err: e }, 'Erreur notif username amis:');
            }
            try {
                for (const room of socket.rooms) {
                    if (room.startsWith('server:')) {
                ...
                logger_1.logger.warn({ err: e }, 'Erreur notif username serveurs:');
            }
        }
        catch (error) {
    ...
    socket.on('VOICE_JOIN', async (data) => {
        ...
        try {
            ...
            logger_1.logger.warn({ err }, 'VOICE_JOIN permission check error');
            return;
        }
        ...
        try {
            ...
            if (!stillOnline) {
                ...
                serviceProxy.users.updateLastSeen(userId)
                    .catch((err) => logger_1.logger.warn({ err }, `updateLastSeen failed for ${userId}`));
            }
        }
        catch (error) {
...
// ============ AUTO-IDLE : détection d'inactivité côté gateway ============
{
    ...
    setInterval(async () => {
        seen.clear();
        for (const [, client] of connections_1.connectedClients) {
            const { userId } = client;
...
serverNodesNs.on('connection', async (nodeSocket) => {
    ...
    const setupCode = (() => {
        ...
        const bytes = (0, node_crypto_1.randomBytes)(14);
        let code = '';
        for (let i = 0; i < 14; i++)
    ...
    nodeSocket.on('NODE_BROADCAST', (data) => {
        ...
        if (payload?.serverId && payload.serverId !== serverId) {
            logger_1.logger.warn(`NODE_BROADCAST rejeté: node ${serverId} tente de broadcast vers serveur ${payload.serverId}`);
            return;
        }
        switch (event) {
            // ── Messages ──
...
async function broadcastPresenceUpdate(userId, status, friends, text, emoji) {
    ...
    for (const friend of friends) {
        ...
        if (fid)
            io.to(`user:${fid}`).emit('PRESENCE_UPDATE', presencePayload);
    }
...
httpServer.listen(env_1.PORT, async () => {
    ...
    const GATEWAY_URL = process.env.GATEWAY_PUBLIC_URL || `http://localhost:${env_1.PORT}`;
    gateway_registry_1.gatewayRegistry.register({ id: GATEWAY_ID, name: GATEWAY_NAME, url: GATEWAY_URL, isSelf: true });
    monitoring_db_1.monitoringDB.upsertGateway({ id: GATEWAY_ID, name: GATEWAY_NAME, url: GATEWAY_URL, enabled: true }).catch(() => { });
    // Charger les autres gateways depuis la DB
    const storedGateways = await monitoring_db_1.monitoringDB.loadGateways();
    for (const gw of storedGateways) {
        if (gw.id !== GATEWAY_ID)
            gateway_registry_1.gatewayRegistry.register({ ...gw });
    }
