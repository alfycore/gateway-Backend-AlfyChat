...
async function proxyRequest(targetUrl, req, res) {
    try {
        ...
        if (contentType && contentType.includes('application/json')) {
            ...
            logger_1.logger.error(`Service ${targetUrl} retourne du non-JSON:`, await response.text());
            res.status(response.status).json({ error: 'Service non disponible' });
        }
    }
    catch (error) {
        logger_1.logger.error('Erreur proxy:', error);
...
app.post('/api/friends/request', async (req, res) => {
    ...
    try {
        ...
        logger_1.logger.error('Erreur envoi demande ami:', error);
        res.status(502).json({ error: 'Service indisponible' });
    }
});
...
app.post('/api/friends/requests/:requestId/accept', async (req, res) => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (fromUserId)
                io.to(`user:${fromUserId}`).emit('FRIEND_ACCEPT', { type: 'FRIEND_ACCEPT', payload: data, timestamp: new Date() });
            if (toUserId)
                io.to(`user:${toUserId}`).emit('FRIEND_ACCEPT', { type: 'FRIEND_ACCEPT', payload: data, timestamp: new Date() });
        }
    }
    catch (error) {
        logger_1.logger.error('Erreur accept ami:', error);
        res.status(502).json({ error: 'Service indisponible' });
    }
});
...
app.get('/api/servers', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Erreur proxy GET /api/servers:', error);
        res.status(502).json({ error: 'Service indisponible' });
    }
});
...
async function proxyToNode(nodeEndpoint, nodePath, req, res) {
    try {
        ...
        logger_1.logger.error('Erreur proxy node:', error);
    ...
    try {
        ...
        req.on('end', async () => {
            try {
                ...
                logger_1.logger.error('Erreur proxy node multipart:', error);
                res.status(502).json({ error: 'Server node indisponible' });
            }
        });
    }
    ...
    try {
        ...
        req.on('end', async () => {
            try {
                ...
                logger_1.logger.error('Erreur proxy multipart service:', err);
                res.status(502).json({ error: 'Service indisponible' });
            }
        });
    }
    ...
    try {
        ...
        req.on('end', async () => {
            try {
                ...
                logger_1.logger.error('Erreur proxy media:', error);
...
app.get('/uploads/*', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Erreur proxy uploads:', error);
        res.status(502).json({ error: 'Service média indisponible' });
    }
});
// Health check
...
io.use(async (socket, next) => {
    try {
        ...
        const decoded = jsonwebtoken_1.default.verify(token, process.env.JWT_SECRET || 'alfychat-super-secret-key-dev-2026');
        ...
        logger_1.logger.error('Erreur d\'authentification WebSocket:', error);
...
async function checkServerPermission(userId, serverId, requiredPerms) {
    try {
        ...
        logger_1.logger.warn('checkServerPermission error:', err);
        return false;
    }
}
...
io.on('connection', async (socket) => {
    ...
    socket.on('message:send', async (data) => {
        ...
        try {
            ...
            if (message.archiveEvent) {
                const archiveEvent = message.archiveEvent;
    ...
    socket.on('message:edit', async (data) => {
        try {
            const updated = await serviceProxy.messages.updateMessage(data.messageId, data.content, userId);
            if (!updated)
                return;
        ...
        try {
            ...
            if (messages && messages.length > 0) {
                try {
                    ...
                    logger_1.logger.warn('Erreur cache messages archivés:', e);
                }
            }
    ...
    socket.on('CALL_REJOIN', (data) => {
        try {
            ...
            logger_1.logger.warn('CALL_REJOIN error:', error);
        }
    });
    // ============ SERVEURS P2P ============
    ...
    socket.on('SERVER_OWNER_JOINED', async (data) => {
        try {
            ...
            logger_1.logger.warn('SERVER_OWNER_JOINED error:', error);
        }
    });
    ...
    socket.on('PROFILE_UPDATE', async (data) => {
        try {
            ...
            try {
                ...
                logger_1.logger.warn('Erreur notification profil amis:', e);
            }
        }
        catch (error) {
    ...
    socket.on('disconnect', async (reason) => {
        ...
        try {
            ...
            try {
                ...
                logger_1.logger.warn(`Impossible de notifier les amis pour ${userId}:`, friendsError);
            }
            ...
            try {
                ...
                logger_1.logger.warn(`Impossible de mettre à jour last_seen pour ${userId}:`, updateError);
            }
        }
        catch (error) {
            logger_1.logger.error(`Erreur lors de la déconnexion de ${userId}:`, error);
        }
    });
});
...
serverNodesNs.use(async (socket, next) => {
    try {
        ...
        logger_1.logger.error('Erreur authentification server-node:', err?.message || err);
...
serverNodesNs.on('connection', async (nodeSocket) => {
    ...
    if (nodeSocket.registerMode) {
        ...
        try {
            ...
            logger_1.logger.error('Erreur lors de l\'auto-enregistrement du server-node:', err);
...
async function broadcastPresenceUpdate(userId, status, friends) {
    for (const friend of friends) {
...
httpServer.listen(PORT, () => {
    logger_1.logger.info(`🚀 Gateway AlfyChat démarré sur le port ${PORT}`);
