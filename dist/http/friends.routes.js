    ...
    app.post('/api/friends/:targetId/block', async (req, res) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                runtime_1.runtime.io?.to(`user:${userId}`).emit('USER_BLOCKED', { blockedId: targetId });
                runtime_1.runtime.io?.to(`user:${targetId}`).emit('YOU_WERE_BLOCKED', { byUserId: userId });
            }
            res.status(response.status).json({ success: response.ok, ...(data ?? {}) });
        }
        catch (error) {
    ...
    app.post('/api/friends/:targetId/unblock', async (req, res) => {
        ...
        try {
            ...
            if (response.ok) {
                (0, block_cache_1.invalidateBlockCache)(userId, targetId);
                // Notifier les deux parties en temps réel
                runtime_1.runtime.io?.to(`user:${userId}`).emit('USER_UNBLOCKED', { unblockedId: targetId });
                runtime_1.runtime.io?.to(`user:${targetId}`).emit('YOU_WERE_UNBLOCKED', { byUserId: userId });
            }
            res.status(response.status).json({ success: response.ok, ...(data ?? {}) });
        }
        catch (error) {
