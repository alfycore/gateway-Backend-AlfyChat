...
function registerAdminRoutes(app) {
    ...
    app.post('/api/admin/services', async (req, res) => {
        ...
        const { rawKey, hash } = (0, registry_1.generateServiceKey)();
    ...
    app.put('/api/admin/services/:id/endpoint', async (req, res) => {
        ...
        const ok = service_registry_1.serviceRegistry.updateEndpoint(decodedId, endpoint.trim());
        if (!ok)
            return res.status(404).json({ error: 'Instance introuvable' });
        monitoring_db_1.monitoringDB.updateServiceEndpoint(decodedId, endpoint.trim()).catch(() => { });
        logger_1.logger.info(`Admin: endpoint de "${decodedId}" mis à jour → ${endpoint.trim()}`);
    ...
    app.post('/api/internal/service-error', async (req, res) => {
        const { sendMail } = await Promise.resolve().then(() => __importStar(require('../utils/mailer')));
        const { ADMIN_ALERT_EMAILS, INTERNAL_SECRET: SECRET } = await Promise.resolve().then(() => __importStar(require('../config/env')));
        const internalSecret = req.headers['x-internal-secret'];
        if (!internalSecret || internalSecret !== SECRET) {
            return res.status(401).json({ error: 'Non autorisé' });
        }
        const { serviceId, errorType, message, stack, severity } = req.body;
        ...
        try {
            await monitoring_db_1.monitoringDB.createIncident({
                title,
            ...
            logger_1.logger.error({ err: dbErr }, 'service-error: erreur insertion DB');
        }
        const mailBody = [
        ...
        logger_1.logger.error({ serviceId, errorType, message }, '[service-error] incident créé');
        res.json({ success: true });
    });
    ...
    app.get('/api/admin/lb/gateways', async (req, res) => {
        if (!await (0, admin_guard_1.requireAdmin)(req, res))
            return;
        const mem = gateway_registry_1.gatewayRegistry.getAll();
        const online120s = 120_000;
        const gateways = mem.map(g => ({
            ...g,
            online: gateway_registry_1.gatewayRegistry.isOnline(g.id, online120s),
    ...
    app.post('/api/admin/lb/gateways', async (req, res) => {
        ...
        const entry = gateway_registry_1.gatewayRegistry.register({ id, name, url, enabled: true });
        monitoring_db_1.monitoringDB.upsertGateway({ id, name, url, enabled: true }).catch(() => { });
        logger_1.logger.info(`Admin: gateway "${id}" ajouté`);
    ...
    app.patch('/api/admin/lb/gateways/:id', async (req, res) => {
        ...
        if (name !== undefined || url !== undefined)
            gateway_registry_1.gatewayRegistry.update(gid, { name, url });
        if (enabled !== undefined)
            gateway_registry_1.gatewayRegistry.setEnabled(gid, !!enabled);
        monitoring_db_1.monitoringDB.patchGateway(gid, { name, url, enabled }).catch(() => { });
        logger_1.logger.info(`Admin: gateway "${gid}" mis à jour`);
        res.json({ success: true });
    });
    ...
    app.delete('/api/admin/lb/gateways/:id', async (req, res) => {
        ...
        const ok = gateway_registry_1.gatewayRegistry.remove(gid);
        if (!ok)
            return res.status(404).json({ error: 'Gateway introuvable' });
        monitoring_db_1.monitoringDB.deleteGateway(gid).catch(() => { });
        res.json({ success: true });
    });
    ...
    app.get('/api/admin/lb/services', async (req, res) => {
        if (!await (0, admin_guard_1.requireAdmin)(req, res))
            return;
        const instances = registry_1.lbRegistry.getAll().map(e => ({
            ...e,
            score: registry_1.lbRegistry.computeScore(e),
        }));
        res.json({ instances });
    });
    /**
    ...
    app.post('/api/admin/lb/services', async (req, res) => {
        ...
        if (registry_1.lbRegistry.getById(String(id))) {
            return res.status(409).json({ error: `Un service avec l'id "${id}" existe déjà` });
        }
        const { rawKey, hash } = (0, registry_1.generateServiceKey)();
        registry_1.lbRegistry.preRegister({
            id: String(id),
            serviceType: serviceType,
            location: String(location).toUpperCase(),
            enabled: true,
        ...
        service_keys_1.allowedServiceIds.add(String(id));
        service_keys_1.bannedServiceIds.delete(String(id));
        logger_1.logger.info(`Admin: service LB "${id}" créé — clé générée`);
        res.status(201).json({
            success: true,
            serviceKey: rawKey,
    ...
    app.post('/api/admin/lb/services/:id/rotate-key', async (req, res) => {
        ...
        if (!registry_1.lbRegistry.getById(sid))
            return res.status(404).json({ error: 'Service introuvable' });
        const { rawKey, hash } = (0, registry_1.generateServiceKey)();
        registry_1.lbRegistry.addKeyHash(sid, hash);
        monitoring_db_1.monitoringDB.storeServiceKeyHash(sid, rawKey).catch(() => { });
        logger_1.logger.info(`Admin: clé rotée pour service "${sid}"`);
        res.json({ success: true, serviceKey: rawKey });
    });
    ...
    app.delete('/api/admin/lb/services/:id', async (req, res) => {
        if (!await (0, admin_guard_1.requireAdmin)(req, res))
            return;
        const sid = decodeURIComponent(req.params.id);
        const ok = registry_1.lbRegistry.remove(sid);
        if (!ok)
            return res.status(404).json({ error: 'Service introuvable' });
        service_keys_1.bannedServiceIds.add(sid);
        service_keys_1.allowedServiceIds.delete(sid);
        monitoring_db_1.monitoringDB.removeServiceInstance(sid).catch(() => { });
        logger_1.logger.info(`Admin: service LB "${sid}" supprimé`);
        res.json({ success: true });
    });
    ...
    app.patch('/api/admin/lb/services/:id', async (req, res) => {
        ...
        if (typeof enabled === 'boolean') {
            registry_1.lbRegistry.setEnabled(sid, enabled);
            monitoring_db_1.monitoringDB.setInstanceEnabled(sid, enabled).catch(() => { });
            if (enabled)
                service_keys_1.bannedServiceIds.delete(sid);
            else
                service_keys_1.bannedServiceIds.add(sid);
        }
        if (endpoint && typeof endpoint === 'string') {
            registry_1.lbRegistry.updateEndpoint(sid, endpoint);
            monitoring_db_1.monitoringDB.updateServiceEndpoint(sid, endpoint).catch(() => { });
        }
        res.json({ success: true });
    });
    ...
    app.post('/api/admin/lb/services/:id/restore', async (req, res) => {
        if (!await (0, admin_guard_1.requireAdmin)(req, res))
            return;
        const sid = decodeURIComponent(req.params.id);
        const ok = registry_1.lbRegistry.restoreInstance(sid);
        if (!ok)
            return res.status(404).json({ error: 'Service introuvable' });
        res.json({ success: true });
    });
    ...
    app.post('/api/admin/lb/reload-keys', async (req, res) => {
        ...
        for (const row of rows) {
            if (row.serviceKeyHash) {
                registry_1.lbRegistry.addKeyHash(row.id, row.serviceKeyHash);
                service_keys_1.serviceKeyHashes.set(row.id, row.serviceKeyHash);
                count++;
            }
        }
        logger_1.logger.info(`Admin: ${count} clé(s) rechargée(s) depuis la DB`);
