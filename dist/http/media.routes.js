...
function resolveMediaEndpoint(preferredLocation) {
    ...
    const instance = service_registry_1.serviceRegistry.selectBestByLocation('media', preferredLocation)
        ?? service_registry_1.serviceRegistry.getAll().find(i => i.serviceType === 'media' && !i.isLocal)
        ?? service_registry_1.serviceRegistry.getAll().find(i => i.serviceType === 'media')
        ?? null;
    ...
    app.get('/api/media/:location/:serviceId/:folder/:filename', async (req, res) => {
        ...
        if (!instance) {
            const regional = service_registry_1.serviceRegistry.selectBestByLocation('media', location)
                ?? service_registry_1.serviceRegistry.getAll().find(i => i.serviceType === 'media' && i.location.toUpperCase() === location.toUpperCase() && !isUnreachableLocal(i))
                ?? service_registry_1.serviceRegistry.getAll().find(i => i.serviceType === 'media' && !isUnreachableLocal(i));
            if (regional) {
                logger_1.logger.warn(`MediaProxy: instance ${serviceId} introuvable, fallback sur ${regional.id}`);
                instance = regional;
            }
        }
