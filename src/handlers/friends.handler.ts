...
import { AuthenticatedSocket, Friendship } from '../types';
...
export function registerFriendsHandlers(
  ...
  socket.on('FRIEND_ACCEPT', async (data) => {
    try {
      const friendship = await serviceProxy.friends.acceptFriendRequest(data.requestId, userId) as Friendship;
  ...
  socket.on('FRIEND_REJECT', async (data) => {
    try {
      ...
      emitToSocket(socket, 'FRIEND_REJECT_SUCCESS', { requestId: data.requestId });
    } catch (error) {
      emitError(socket, 'FRIEND_REJECT_ERROR', error);
    }
  });
  ...
  socket.on('FRIEND_REMOVE', async (data) => {
    try {
      ...
      emitError(socket, 'FRIEND_REMOVE_ERROR', error);
    }
  });
  ...
  socket.on('USER_BLOCK', async (data) => {
    try {
      ...
      emitToSocket(socket, 'USER_BLOCK_SUCCESS', { blockedUserId: data.blockedUserId });
    } catch (error) {
      emitError(socket, 'USER_BLOCK_ERROR', error);
    }
  });
  ...
  socket.on('USER_UNBLOCK', async (data) => {
    try {
      ...
      emitToSocket(socket, 'USER_UNBLOCK_SUCCESS', { unblockedUserId: data.blockedUserId });
    } catch (error) {
      emitError(socket, 'USER_UNBLOCK_ERROR', error);
    }
  });
}
